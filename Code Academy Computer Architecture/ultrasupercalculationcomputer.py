...
class UltraSuperCalculator:
  def __init__(self,name)->None:
    ...
    self.history_registers = [0 for i in range(0,9)]  
    self.numbers_index =1
    self.history_index = 0
    self.temp_history_index=0
    self.user_display =""
    self.update_display(f'hey {self.name}')
  ...
  def store_value_to_register(self,value_to_store):
    ...
    #print(f"{value_to_store} was stored at register {self.numbers_index} which now contains {self.number_registers[self.numbers_index]}")
    self.numbers_index+=1
  ...
  def store_to_history_register(self,result_to_store):
    #print(f"history index before {self.history_index}")
    if (self.history_index > 9):
      self.history_index=0
    self.history_registers[self.history_index]=bin(result_to_store)
    #print(f"data stored at history index {self.history_index}")
    #print(f"history index before corrections {self.history_index}, temp history {self.temp_history_index}")
    self.history_index +=1
    self.temp_history_index=self.history_index
    #print(f"history index after corrections {self.history_index}, temp history {self.temp_history_index}")
  ...
  def get_last_calculation(self):
    ...
    last_value =f"last result was {int(self.history_registers[self.temp_history_index],2)} from index {self.temp_history_index} "
  ...
  def binary_reader(self, instruction):
    ...
    if (opcode == '000001'):
      ...
      self.update_display(f"Invalid OPCODE {opcode}!")
      return
    ...
    if (function_code == '100000'):
      ...
      self.update_display(f"Invalid function code {function_code}!")
      return
...
calc = UltraSuperCalculator("beny")
#calc.store_value_to_register("111")
#print(calc.load_value_from_register("001"))
#calc.binary_reader("1234567812345678123456781234567")
#calc.binary_reader("12345678123456781234567812345678")

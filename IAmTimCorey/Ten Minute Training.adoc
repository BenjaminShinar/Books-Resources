...
=== Saving Money by Deleting Azure Resource the Right Way

.Ensuring that all related resources are also deleted from Azure
[%collapsible]
====
using resource groups as a logical partition (container) that holds all related resources. when we delete a resource group, all the resources in the group and all dependent resources will also be deleted.
====

=== Updating to .NET 9, C# 13, and Visual Studio 17.12

.Updating tools
[%collapsible]
====
we don't have to open a solution/project to update, we can check the notifications to see if there are updates, or under the "help" menu and check for new versions.

Alternatively, we can look at the visual studio installer and see if there are updates or use the preview version instead of the public release version. this will give us the option for .Net 8.0 (long term support) and .Net 9.0 (standard support) frameworks side by side.

if we aren't using visual studio, we can download SDKs manually from dotnet website. we can also get just the runtime if we just need to run stuff.
====
...
The TimeSpan object is a period (time between time values), and it got an update in .Net 9. the existing overload uses the double type, and it has some precision issues. there is a new overload that takes the arguments as different parameters of type long, which avoids the floating point problem.

[source, csharp]
----
TimeSpan demoWrong = TimeSpan.FromSeconds(101.832); // double

Console.WriteLine(demoWrong); // not expected value

TimeSpan demoRight = TimeSpan.FromSeconds(101, 832); // long, long

Console.WriteLine(demoRight); // as expected
----
...
.Getting Data from a configuration file in Blazor
[%collapsible]
====

Getting information from configuration files into dependency injection.
We have an "appSettings.Json" file, and a `CloudInfoOptions` class. we want to inject the class to our blazor page. we need to bind it into the blazor services.
...
builder.Service.AddOptions<CloudInfoOptions>()
...
we can have the options exposed as a singleton, a separate object per request (snapshot), or a monitored options that we can listen to changes in the file. we can also have custom code to validate the file, like requiring all fields to exist.
====

=== Import A SQL Database In Visual Studio and Refactor It

.Import and manage databases
[%collapsible]
====

using a "SQL Server Database Project" template. we can import an existing database with the schema. this doesn't import data, just the structure.
we can now rename fields, target different platforms such as Azure (which will generate a change script).
...
.New features
* adding breakpoint on the closing braces of the method to see the return value.
* getting github permanent link to code from context (right click).
* copy files between visual studio instances.
* better copy of errors from error list, the old behavior is "copy row", and the "copy" action copies the description.
...
Blazm.extension for visual studio and Blazor, context menu, moving from code-behind to the ".razor" file, finding usages in components, generating isolated files for css and javascript, generating unit test files (C# or Razor syntax), showing project routes and some other useful commands.
...
.converting solution files from sln to slnx
[%collapsible]
====

replacing dot net sln solution files with the new slnx files. the solution file contains the version, minimum version, projects in the solution, build configurations, references to other solutions, and other stuff. this file is prone to merge errors.
In new versions of visual studio, we can enable the 'persistent solution file' option to work with the new slnx format. we can save the file as slnx and look at the differences. it is much simpler, only having the paths to the projects and less boilerplate code.

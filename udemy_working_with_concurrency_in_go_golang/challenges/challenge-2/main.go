...
Five philosophers, numbered from 0 through 4, live in a house where the table is laid for them; each philosopher has their own place at the table.
Their only difficulty – besides those of philosophy – is that the dish served is a very difficult kind of spaghetti which has to be eaten with two forks. There are two forks next to each plate, so that presents no difficulty.
...
func main() {
	...
	for i, name := range finishedOrder {
		fmt.Printf("the %d: to finish eating was %s\n", i+1, name)
	}
...
func diningProblem(philosopher Philosopher, wg *sync.WaitGroup, forks map[int]*sync.Mutex, seated *sync.WaitGroup) {
	...
	doneEating.Lock()
	finishedOrder = append(finishedOrder, philosopher.name)
	doneEating.Unlock()
}
